 ...
 * Commit `.R`, `.Rd` and `.RData` files to GitHub.
...
NULL
```
After running `roxygenize()` on the the package directory an `.Rd` file is produced.
...
\references{
Cronin et al. 2007. An assessment of population size and distribution of harbor seals in the Republic of Ireland during the moult season in August 2003. Journal of zoology, 273: 131-139

Zuur et al. 2009. Mixed effects models and extensions in ecology with R. Springer. ISBN: 978-0-387-87457-9.
}
```

Bump version. It should generally be `1.<minor>.<patch>`. Use `<minor>` when adding datasets and `<patch>` when you're fixing typos and other minor stuff.

To make sure everything checks out, run the check from one level above `AED/` folder.

```{bash}
R CMD build AED
R CMD check AED_<version>.tar.gz
```

Warnings about datasets not being used in the code can be ignored.

To try out if everything worked, you can

```{bash}
R CMD INSTALL AED_<version>.tar.gz
R
load(AED)
data(<dataset name>)
```